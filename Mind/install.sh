...
# ---------------------------------------------------------------------------
# Python daemon dependencies (pydbus, requests, numpy for the hash embedder)
# ---------------------------------------------------------------------------
info "Installing Python daemon dependencies…"
if ! python3 -c "import pydbus" 2>/dev/null; then
    python3 -m pip install --user --quiet pydbus requests numpy
    info "pip packages installed."
else
    info "pydbus already available."
    python3 -c "import numpy" 2>/dev/null \
        || python3 -m pip install --user --quiet numpy \
        || warn "numpy unavailable — memory will degrade to lexical recall."
fi
...
cp "${SCRIPT_DIR}/daemon/mind_memory.py" "${DAEMON_LIB}/mind_memory.py"
chmod +x "${DAEMON_LIB}/mind-daemon.py"

# ---------------------------------------------------------------------------
# Memory seed — copy original_memory/ into the daemon's data dir so the
# letter (and any other first-memory) becomes the mind's first instruction.
# ---------------------------------------------------------------------------
if [ -d "${SCRIPT_DIR}/../original_memory" ]; then
    info "Seeding memory from ${SCRIPT_DIR}/../original_memory…"
    mkdir -p "${HOME}/.local/share/mind/original_memory"
    cp -r "${SCRIPT_DIR}/../original_memory/." "${HOME}/.local/share/mind/original_memory/"
fi
