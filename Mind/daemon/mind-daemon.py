...
class BubbleDaemon:
    ...
    def __init__(self) -> None:
        ...
        log.info(
            "Mind daemon up  ctx_size=%d  nudge_idle_min=%s  base=%s  model=%s",
    ...
    def SendMessage(self, message: str, token: str, context: str) -> None:
        ...
        with self._lock:
            ...
            self._last_user_time = time.monotonic()
            self._trim_history()
        GLib.idle_add(lambda: self._stream(token))
    ...
    def _stream(self, token: str) -> None:
    ...
    def _consume_sse(self, resp, token: str) -> None:
